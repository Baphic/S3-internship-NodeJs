            ...
            bcrypt.compare(
              ...
              (error, verificacionPassword) => {
                ...
                if (verificacionPassword) {
                  ...
                  return res
                    .status(500)
                    .send({ mensaje: "Contraseña y/o usuario incorrecto" });
                }
              }
            );
          } else {
  ...
  if (ring == "Admin") {
    Usuario.findById(idReq, (error, reqEnc) => {
      ...
      if (reqEnc.rol == "Admin")
        return res
          .status(500)
          .send({ requester: "Este usuario ya es rol Admin" });
  ...
  if (run == "Admin") {
    Usuario.findById(idAdm, (error, uEn) => {
      ...
      if (uEn.rol == "Requester")
        return res
          .status(500)
          .send({ requester: "Este usuario ya es rol Requester" });
  ...
  Solicitudes.find(
    ...
    (error, allSo) => {
      ...
      if (!allSo)
        return res
          .status(500)
          .send({ mensaje: "Error, no se encontraron Solicitudes" });
